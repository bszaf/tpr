...
void fill_message(struct message_s *m, size_t msg_size) {
  char fill_with = 'a';
  memset(m->data, fill_with, msg_size);
}
...
int main(int argc, char** argv) {
  ...
  if (world_rank == 0) {
    ...
    fill_message(&send_msg, MSG_SIZE);
    ...
    for (int i = 0; i < parts; i++) {
      MPI_Send(&send_msg, 1, mpi_message_type, 1, tag, MPI_COMM_WORLD);
      MPI_Recv(&recv_msg, 1, mpi_message_type, 1, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    MPI_Barrier(MPI_COMM_WORLD);
    for (int i = 0; i < parts; i++) {
      MPI_Recv(&recv_msg, 1, mpi_message_type, 0, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Send(&send_msg, 1, mpi_message_type, 0, tag, MPI_COMM_WORLD);
    }
